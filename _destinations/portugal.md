---
layout: destination

permalink: /destinations/portugal/

# jekyll-seo-tag
# see https://github.com/jekyll/jekyll-seo-tag/blob/master/docs/advanced-usage.md
canonical_url: https://lilyroger.com/destinations/portugal/

date: 2026-03-07 16:00:00 +0800

# article:date_modified
# see _includes/head.html
date_modified: 2026-03-07 16:00:00 +0800

# country name in pinyin for nature sorting
# see _pages/destinations.md
title_pinyin: putaoya

# 保持 title 为 SEO 服务（长标题）。自定义的 distitle_display 变量定制文章页面的 <h1> 显示
title_display: 葡萄牙

tag: 葡萄牙

# 保持 title 为 SEO 服务（长标题）。自定义的 title_display 变量定制文章页面的 <h1> 显示
title: 葡萄牙 (Portugal) 旅游攻略；里斯本景点，美食，旅行指南和游记
title_display: 葡萄牙 (Portugal)
description: 这里汇集了我们在葡萄牙里斯本的旅行攻略和指南，从宏伟的热罗尼莫斯大修道院到贝伦区的正宗蛋挞，分享最地道的沙丁鱼美食与波尔图美酒体验。

tagline: 电车的光影，蛋挞的香气，沙丁鱼的烟火和波特酒的回味
image_base: destinations/portugal/lisbon-yellow-tram-28-portugal
image_alt: 穿行在里斯本老城阿尔法玛区的经典28路黄色有轨电车，葡萄牙旅行的标志性景观。
---

{%- assign trip = site.trips | where: "path", "_trips/2025-us-italy.md" | first -%}
在我们的[{{ trip.title }}]({% link _trips/2025-us-italy.md %})中，我们选择了葡萄牙里斯本作为从美国前往意大利之前中转停留的城市，用三天时间游览了这座“七丘之城”的精粹，从宏伟的热罗尼莫斯大修道院 (Jerónimos Monastery)，到修缮中的贝伦塔 (Belém Tower)。当然也不会错过贝伦老店的正宗蛋挞，街头的炭烤沙丁鱼，更喜欢上了美味的波特酒 (Port Wine)。虽然因为南欧大停电，我们被困在了里斯本，在机场多住了两天，但我们的里斯本之行仍然近乎完美。